...
class AlignedResize(object):
    ...
    def _resize_depth(self, results):
        for key in results.get('depth', []):
            if self.keep_ratio:
                depth = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
                depth = self._align(depth, self.size_divisor, interpolation='nearest')
            else:
                depth = mmcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
                h, w = depth.shape[:2]
                assert int(np.ceil(h / self.size_divisor)) * self.size_divisor == h and \
                       int(np.ceil(w / self.size_divisor)) * self.size_divisor == w, \
                    "depth size not align. h:{} w:{}".format(h, w)
            results[key] = depth
